...
function ProfileFormDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose() }}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{profile ? `Edit — ${profile.full_name}` : 'New User'}</DialogTitle>
        </DialogHeader>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Manage system accounts. New users must first be created via Supabase Auth, then their profile appears here.
          </p>
        </div>
        <Button size="sm" variant="outline" className="gap-1.5" onClick={() => { setEditing(null); setDialogOpen(true) }}>
          <UserPlus className="h-4 w-4" />
          Note: Create via Supabase Auth
        </Button>
      </div>
              ...
              {!isLoading && profiles?.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-muted-foreground">
                    No user profiles yet. Create users via Supabase Auth Dashboard, then seed their profiles.
                  </td>
                </tr>
              )}
            </tbody>
